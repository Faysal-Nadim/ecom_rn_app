...
export const verifyEmail = (data) => {
  return async (dispatch) => {
    try {
      dispatch({ type: verifyConstant.EMAIL_REQUEST });
      const res = await axiosInstance.post(`/user/verification/email`, data);
      if (res.status === 202) {
        dispatch({
          type: verifyConstant.EMAIL_SUCCESS,
      ...
      dispatch({
        type: verifyConstant.EMAIL_FAILURE,
  ...
  return async (dispatch) => {
    try {
      ...
      const res = await axiosInstance.post(
        `/user/verification/email/verify`,
        data
      );
      if (res.status === 202) {
        dispatch({
